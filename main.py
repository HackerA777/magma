...
@cuda.jit(numba.void(numba.uint8[:], numba.uint8[:], numba.uint8[:, :]), device=True)
def magma_T(in_data: np.uint8, out_data: np.uint8, table: np.ndarray):
...
def encrypt(round_keys: np.array, block: np.uint8, out_block: np.uint8, table: np.array):
    t = cuda.local.array(8, numba.uint8)
...
def main():
    ...
    block = main_block
    # block = np.array([16, 50, 84, 118, 152, 186, 220, 254] * 1310720, dtype=np.uint8)
    # main_block = block
